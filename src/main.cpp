...
void mqtt_callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(String(topic) == "remotest/ledColors") {
    if(payloadS == "red") {
      lv_led_toggle(led);
    }
    if(payloadS == "blue") {
...
void setup() {
  ...
  init_gui_elements();
  init_sideled();
  mqtt_publish("remo/apartment/room1/color", "red");
  mqtt_publish("remo/apartment/room2/color", "purple");
  mqtt_publish("remo/apartment/room3/color", "green");
  mqtt_publish("remo/apartment/room4/color", "red");
}